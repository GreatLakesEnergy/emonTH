...
### GENERAL SETTINGS ###
SERIALPORT = "/dev/ttyO2"  # the default com/serial port the receiver is connected to
...
def waitForHandshake(isEOF=False):
  ...
  while True:
    if millis()-now < 4000:
      ...
      if len(rx) > 0:
        print ("Moteino reiceved data: [" + rx + "]")
        if rx == "FLX?OK":
          print ("HANDSHAKE OK!")
          return True
        elif rx == "FLX?NOK":
      ...
      if waitForHandshake():
        ...
        while seq < len(content):
          ...
          if isEOF==False:
            ...
            def Mbox(title, text, style):
              ctypes.windll.user32.MessageBoxA(0, text, title, style)
            Mbox('RESULT', 'FAIL, IMG REFUSED BY TARGET!', 0)
            exit(99);
